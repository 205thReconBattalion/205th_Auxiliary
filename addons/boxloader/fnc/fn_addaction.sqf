...
_ply addAction ["Select loading target",{
	_veh = cursorObject;
	params ["_ply"];
	if (
		(vehicleCargoEnabled _veh) &&
		(((_veh modelToWorld (_veh getVariable ["boxloader_load_pos",[0,0,0]])) distance _ply)<15)
	) then {
		_ply setVariable["boxloader_tgt",_veh,true];
	};
},[],0,false,true,"","[_this,_target,cursorObject] call boxloader_fnc_cond_select"];

_ply addAction ["Select loader",{
	_veh = cursorObject;
	params ["_ply"];
	if (
		(_veh getVariable ["boxloader_load_weight",0]>0) &&
		(((_veh modelToWorld (_veh getVariable ["boxloader_load_pos",[0,0,0]])) distance _ply)<15)
	) then {
		_ply setVariable["boxloader_crane",_veh,true];
	};
},[],0,false,true,"","[_this,_target,cursorObject] call boxloader_fnc_cond_loader"];

_ply addAction ["Load this in target",{
	_veh = (_this select 0 getVariable ["boxloader_tgt",objNull]);
	_box = cursorObject;
	if (
		((_veh canVehicleCargo _box) select 0) &&
		((_box distance (_veh modelToWorld (_veh getVariable ["boxloader_load_pos",[0,0,0]])))<15)
	) then {
		[_veh,_box] call boxloader_fnc_load;
	};},[],5,false,true,"","[_this,_target,cursorObject] call boxloader_fnc_cond_load"];
...
_ply addAction ["Precise Unload",{
	...
	if ((!isNull _veh) && (_box distance (_this select 0)<10)) then {
		...
		_bedheight = (_veh modelToWorld (_veh selectionPosition _bedHeight));
		_pos = (_veh modelToWorld (_veh selectionPosition _pos));
