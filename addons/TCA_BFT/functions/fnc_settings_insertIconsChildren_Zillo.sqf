...

{
	...
	_icon = _x;
	//_name = (_names select _i);//getText (configfile >> "CfgMarkers" >> _markerSide + "_" + _icon >> "Name");
	_name = ((GVAR(bft_fireteamInteractions) select _i) select 0);
	//_interactIcon = (_icons select _i);//getText (configfile >> "CfgMarkers" >> _markerSide + "_" + _icon >> "icon");
...
} forEach ["RB205_BFT_11_a","RB205_BFT_11_b","RB205_BFT_11_c","RB205_BFT_11_d"];//GVAR(availableMarkerIcons_Zillo);
