...
if !(GVAR(LoadoutDiaryRecord_Enabled)) exitWith {
	if (
		missionNameSpace getVariable ["TCA_Main_LoadoutDiaryRecord", ""] != ""
	) then {
		player removeDiaryRecord ["Diary", GVAR(LoadoutDiaryRecord)]; 
	};
};
...
fn_invString = {
	...
	} forEach ((items player) call FUNC(arrToCount));
